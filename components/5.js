...
function AgenticBubble({ styleType = 6, cameraMode = 'default' }) {
  ...
  const curve = useMemo(() => {
    ...
    for (let i = 0; i <= 100; i++) {
      const t = i / 100
      const angle = t * Math.PI * 2 * 3 // 3바퀴 회전
      const x = Math.sin(angle) * 1.5
      const y = (t - 0.5) * 6 // 수직 방향 길이
  ...
  return (
    <>
      <mesh ref={meshRef} position={[0, 0, 0]} rotation={[0, 0, 0]}>
        <tubeGeometry args={[curve, 200, 1.2, 64, false]} />
  ...
  return (
    ...
    >
      ...
      >
        ...
        <AgenticBubble styleType={6} cameraMode="default" />
      </Canvas>
    </div>
  )
}
